...
function imageFallbackSibling(img: HTMLImageElement): HTMLElement | undefined {
...
function markBrokenImage(img: HTMLImageElement): void {
	...
	const label = img.alt.trim() || 'Broken image';
	const existing = imageFallbackSibling(img);
	if (existing) {
...
function clearBrokenImage(img: HTMLImageElement): void {
	...
	imageFallbackSibling(img)?.remove();
}
