...
## Tool lifecycle, `rawInput`, and JSON-RPC shape

The official ACP documentation does not define CLI flags such as `--verbose` or `--show-paths` that change how `session/update` notifications are structured. Message shape is determined by the agent implementation and the protocol fields it populates.

### Why `rawInput` is often empty on the first `tool_call`

In practice (including Cursor CLI as of early 2026), you may see `rawInput: {}` on the initial `tool_call` while the tool is still `pending` or `in_progress`:

1. **Timing** — The agent can emit `tool_call` as soon as it decides to invoke a tool, before arguments (for example the file path) are fully serialized into the notification.
2. **Completion updates** — When the tool finishes, a `tool_call_update` with `status: "completed"` often carries **`rawOutput`** (file text, terminal `stdout`/`stderr`, search counts, or structured **`content`** such as `diff` blocks). The agent may omit **`rawInput`** on that update to keep payloads smaller.

The protocol does not guarantee that `rawInput` will be mirrored on every update. Rely on **`tool_call_update`** for results and paths when the pending notification was sparse.

### Increasing visibility (no guaranteed effect on stdout JSON-RPC)

These are common workarounds; whether they change agent behavior depends on the Cursor CLI version:

1. **`initialize` / `clientCapabilities`** — You can advertise standard capabilities (for example `fs.readTextFile`) so the client can serve `fs/read_text_file`. Experimental or undocumented capability keys may be ignored. The TypeScript SDK defines [ClientCapabilities](https://agentclientprotocol.com/protocol/initialization#client-capabilities); do not assume undocumented fields are honored.
2. **Environment variables** — Some builds log extra detail to **`stderr`** (for example `DEBUG=agent:*` or `LOG_LEVEL=trace`). That output is separate from JSON-RPC on **`stdout`**; capture stderr if you need it (`agent acp 2> debug.log`).
3. **Infer from completed updates** — Prefer **`tool_call_update`** with **`rawOutput`** or **`content`** (including `type: "diff"` with `path`) to show what actually ran.

## IB Chat integration (this repository)

**IB Chat** is the in-editor ACP client shipped by the **IrishBruse's Utilities** VS Code extension. It spawns the configured agent, runs the `initialize` handshake, and maps `session/update` notifications into the webview (`webview/ib-chat`).

| Area | Location |
| --- | --- |
| Agent process, stdio, `initialize`, FS handlers | `src/chat/acp/acpAgentProcess.ts` |
| `session/update` to webview messages (text, tools, diffs, `rawOutput`) | `src/chat/acp/acpSessionUpdateMapping.ts` |
| Session wiring | `src/chat/acp/acpSessionBridge.ts` |
| Webview tool UI | `webview/ib-chat/src/components/ToolCallBlock.tsx` |

Behavior aligned with this document:

- **Subtitles** — The dim line under a tool title comes from `tool_call` when the agent sends `locations`, text `content`, or non-empty `rawInput`. If the pending `tool_call` had empty `rawInput`, **IB Chat fills the subtitle on `tool_call_update`** from `locations`, from the first **`diff`** block’s `path` when present, or from structured **`rawOutput` / `rawInput`** fields such as `path` / `filePath` (read tools use the tracked `kind` so file names can be shown as a basename when the completion carries a path).
- **Body** — Read/search/terminal/edit outputs are taken from structured **`content`** blocks and from **`rawOutput`** (see mapping implementation).

Standalone dev preview (`webview/ib-chat-standalone`) replays fixtures such as `mock/edits.ndjson` for UI testing.
