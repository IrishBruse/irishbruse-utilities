import type { ToolCallDiffRow } from "../protocol/ibChatProtocol";
 ...
 */
export function computeToolCallDiffRows(oldText: string, newText: string): ToolCallDiffRow[] {
    ...
    while (i > 0 || j > 0) {
        if (rows.length >= maxDiffRows) {
            rows.unshift({ kind: "context", text: "… (diff truncated for display)" });
            break;
        }
