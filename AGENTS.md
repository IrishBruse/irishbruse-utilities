...
- VS Code does not pass the explorer selection into a command invoked by a keybinding; only context-menu invocations receive `resource` as the first argument. Paste Image saves the clipboard image to a temp file first, then runs the built-in `copyFilePath` command and reads the clipboard so the explorer selection path is available without losing the image data.
