...
const promptHistoryMementoKey = "ibUtilities.ibChat.promptHistoryBySession";
...
function readMap(context: ExtensionContext): PromptHistoryMap {
    const raw = context.workspaceState.get<PromptHistoryMap | undefined>(promptHistoryMementoKey);
 ...
 */
export function getIbChatPromptHistoryEntries(context: ExtensionContext, sessionId: string): string[] {
 ...
 */
export function setIbChatPromptHistoryEntries(context: ExtensionContext, sessionId: string, entries: string[]): void {
    if (sessionId.length === 0) {
        return;
    }
    const map = { ...readMap(context) };
 ...
 */
export function removeIbChatPromptHistoryEntries(context: ExtensionContext, sessionId: string): void {
    if (sessionId.length === 0) {
        return;
    }
    const map = { ...readMap(context) };
